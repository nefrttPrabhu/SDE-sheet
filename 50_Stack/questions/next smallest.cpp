...
void smallest(vector<int> arr){
    ...
    for(int i = n-1 ; i >= 0 ; i--)// peeche se chal rhe 3 -> 4 -> 1 -> 2
    {
        ...
        arr[i] = s.top(); //I1:-1 arr mai last index overwrite karlega ie idhar hua ye ki stack mai hume chota element mil gya and usko humne peeche se arr mai dal diye coz array mai kahin se bhi kisiko bhi access kar skte
